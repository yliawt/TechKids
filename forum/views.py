...
def forum(request, tag=None):
    # Get all user posts or filter by tag if provided
    if tag:
        user_posts = UserPost.objects.filter(tag=tag)
    else:
        user_posts = UserPost.objects.all()

    # Retrieve all distinct tags for display in the sidebar
    tags = UserPost.objects.values_list('tag', flat=True).distinct()
    ...
    context = {
        ...
        'tags': tags,
        'selected_tag': tag,  # Pass the selected tag to the template
    }
...
def userPost(request):
    ...
    if request.method == 'POST':
        if form.is_valid():
            ...
            topic = UserPost.objects.create(title=title, tag=tag, author=request.user.author, description=description)
            topic.save()
